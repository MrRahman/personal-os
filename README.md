...
The 18-topic, 87-tag taxonomy is defined in the `/triage` skill. Modify `.claude/skills/triage/SKILL.md` to adjust topics and tags for your interests.

### Adding New Skills
A project skill lives at `.claude/skills/<name>/SKILL.md` (**directory form** — a flat `.claude/skills/<name>.md` will NOT register as a `/<name>` slash command). Each `SKILL.md` has YAML frontmatter (`name`, `description`) plus the instructions Claude follows. After adding or moving one, **restart Claude Code** so it re-indexes. Universal cross-project skills live globally in `~/.claude/skills/` (or `~/.claude/commands/`). The legacy `.claude/commands/<name>.md` flat form still works but the directory form is preferred.
