...
#   - WORK (WorkCo) tools are READ-ONLY: list/get/search only. NO create/update/
...
# WORK connectors: enumerated READ-ONLY tools only (the hard line — never mutate WorkCo).
# Personal/local servers: server-level allow (mutations user-accepted).
ALLOWED_TOOLS=(
  Read Glob Grep Write
  # meeting-note shell pre-creation (STEP 2i) — the ONLY Bash, scoped to one helper.
  # meeting_notes.py is read-only path logic + CREATE-ONLY shell stamping; no deletes.
...
with open(state, "w") as f:
    ...
    f.write(redact_text(json.dumps({"last_run": rec["ts"], "day": datetime.datetime.now().strftime("%Y-%m-%d"),
               "status": rec.get("status"), "cost_usd": rec.get("cost_usd"),
               "denials": rec.get("denials")}, indent=2), _SECRETS))
