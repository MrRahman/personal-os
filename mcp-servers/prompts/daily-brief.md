...
1. **NEVER ask the user anything. NEVER wait for input.** There is no human in the loop. Any step in `/morning-plan` or `/reflect` that says "ask", "confirm", "present for approval", "y/n", "wait for the user", or "Interactive Check-In" is **converted to: do the safe read-only version automatically, or skip it**. You do not create Todoist tasks, you do not create Notion pages, you do not create meeting notes, you do not block on confirmation. You gather, synthesize, and write ONE file: today's daily note.
...
3. **Work (Ripple) Google is STRICTLY READ-ONLY.** Use only the claude.ai connector read tools for work: `mcp__claude_ai_Google_Calendar__list_events`, `mcp__claude_ai_Google_Calendar__get_event`, `mcp__claude_ai_Google_Calendar__list_calendars`, `mcp__claude_ai_Gmail__search_threads`, `mcp__claude_ai_Gmail__get_thread`. **NEVER** call `create_event`, `update_event`, `delete_event`, `respond_to_event`, `create_draft`, `label_*`, or any write/send tool against the work account. No work calendar mutations. No work email sent or drafted. (The wrapper's allowlist also enforces this, but you must not even attempt it.)

4. **Work and personal Google never cross.** Work = claude.ai connector tools, which are pre-scoped to `srahman@ripple.com` (do NOT pass an `email` argument). Personal = `mcp__google-personal__*` tools, which MUST pass `email: "1srahman@gmail.com"`. Calling `google-personal` with the work email 403s; calling the connector for personal data returns the wrong account. Keep them strictly separate and tag every gathered item `[Work]` or `[Personal]`.
...
- Read `CLAUDE.md` sections if needed for conventions (Google Account Mapping, Slack Priority Channels, Calendar Block Conventions, Todoist Projects, Todoist Label → Obsidian Project Map, Goal system, Obsidian Vault Structure). Paths and identity (work email `srahman@ripple.com`, personal `1srahman@gmail.com`, short name "Sul"/"Sulaiman") come from there.
...
| Personal Calendar | `mcp__google-personal__manage_calendar(operation:"agenda", email:"1srahman@gmail.com")` | `skipped += "Personal Google (Calendar) — token likely revoked; re-auth: npx @aaronsb/google-workspace-mcp auth."` |
| Personal Gmail | `mcp__google-personal__manage_email(operation:"search", email:"1srahman@gmail.com", limit 1)` | `skipped += "Personal Google (Gmail) — token revoked; re-auth via manage_accounts → authenticate."` |
...
- **Personal:** `mcp__google-personal__manage_calendar(operation:"agenda", email:"1srahman@gmail.com")` for TODAY. Tag `[Personal]`.
...
- For meetings, capture: start time, title, and attendee names (pull ALL attendees from the invite, not just known People — this is a standing user rule). Mark therapy/"Wylder Appointment" events with discretion: list them on the schedule as `[Personal]` but do NOT cross-post or expand details (per `user_therapy.md`).
...
- **Personal:** `mcp__google-personal__manage_email(operation:"search", email:"1srahman@gmail.com", ...)`, limit ~10; for actionable threads use `operation:"read"`. Tag `[Personal]`.
...
- `mcp__imessage__extract_action_items(hours:24)` for any asks/commitments from messages (esp. wife Bonnie, parents, sisters). Surface as awareness lines under `### Respond — Personal` (e.g., `Bonnie (iMessage): "<preview>" — owes reply`). Do NOT create tasks. Handle with the same discretion rules as the vault.

### 2g. Vault context (read-only)
- **Goals:** read the current-quarter file `/Users/sulaimanrahman/Documents/PersonalOS/Goals/<YYYY-QX>.md` (derive quarter from TODAY; e.g. 2026-06 → `2026-Q2`). Extract goal names, unchecked milestones with target dates (flag any due within 7 days or overdue), `This Week` targets, and Lead Indicators. Flag a goal "Stale" if its `This Week` is empty and there's no obvious recent activity. Also read the annual file `/Users/sulaimanrahman/Documents/PersonalOS/Goals/<YYYY>.md` only if you need the decision filter / anti-goals for a BALANCE flag — otherwise skip to save cost.
...
- **Meetings:** chronological list, `H:MM AM/PM: <title> — <attendees>`. Blocks are listed too (e.g., `7:15 AM: Travel`, `11:00 AM: Focus block (2h)`) but clearly as blocks, not meetings. Do **not** wikilink meeting notes (the unattended brief does not create meeting notes — leave them as plain titles; the user/`/reflect` can create notes later).
...
- **Active Goals:** goal name, next milestone (+ proximity/overdue), stale flag if any, linked projects, and this-week lead-indicator targets — pulled from the quarter file. Mirror the example's bullet style.
...
| Read personal calendar/email via google-personal (email:"1srahman@gmail.com") | ✅ |
...
| Create Todoist tasks, Notion pages, meeting notes, Resource/Topic notes | ❌ not in unattended brief |
