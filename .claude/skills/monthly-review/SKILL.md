...
**Tone (v3.0 Phase 2):** deliver all accountability — goal RAG status, execution trend, anti-goal patterns — in the **coach voice** from `.claude/skills/_conventions.md` → "Coach voice". Keep the metrics; frame RAG as direction not verdict, read an anti-goal cross as the pattern-under-pressure, end with one kind next step. Never a report card.

## Instructions

### 0. Use the pre-built draft (v3.0)

A background `cadence-draft.sh monthly` job may have already written this month's review as a DRAFT: `Monthly Reviews/<YYYY-MM>.md`, frontmatter `status: draft`, auto content inside `<!-- BEGIN/END:auto-draft -->`. **If that draft exists**, make this a thin **adjust → confirm**: read it, present the `(proposed)` RAG per goal + trajectory adjustments for the user to adjust, then write confirmed decisions into the human section (after the block) and flip `status: draft → final`. Re-gather only what's missing; never rewrite the managed block unless asked; never delete the file. If NO draft exists, fall back to the full gather below.
