...
**Vault write safety:** never `rm`/`mv`/delete/rename any file under `~/Documents/PersonalOS` (cloud-synced, no git undo). Flag duplicates or orphans as a `[TODO]`; never fix by deleting. See `_conventions.md` → Vault write safety.
...
**iMessage (if available):** Use `extract_action_items(hours=24)` to scan the last 24 hours of messages for potential requests, commitments, or action items. Feed results into Action Detection (Step 4a) alongside Gmail and Slack results. For messages from wife ([partner]), route proposed tasks to Todoist "Us" project at P2. For messages from parents, route to "Personal" project at P2. For messages from friends, route to "Personal" project at P3.

**Family Pulse (always runs):** After fetching iMessage data, specifically check for recent messages with core family contacts: [partner], parents (the parents), sisters ([sibling], [sibling]). For each, note last message date and whether user was sender or recipient. This feeds the Life Pulse section and enriches People note `last_interaction` tracking in `/reflect`.
...
- **Relationship indicators:** Read People notes for core family members ([partner], [parent], [parent], [sibling], [sibling]) and check `last_interaction` dates. Fetch these regardless of whether family members appear in today's meetings.
...
- If no couple time logged this week and there's an evening free → suggest: "Date night with [partner] (7-9 PM)"
...
4. **"Block time?"** — Offer both work focus blocks AND personal calendar items based on Lead Indicator Pulse data:
   ...
   - Personal blocks → create on personal calendar via `manage_calendar(operation: "create")` on google-personal MCP, or on "[partner] + Sul Calendar" for shared events. Name: "[activity]" (not "Focus:" prefix for personal items)
