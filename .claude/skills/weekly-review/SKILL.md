...
**Tone (v3.0 Phase 2):** deliver all accountability — execution score, missed targets, aging waiting-on — in the **coach voice** from `.claude/skills/_conventions.md` → "Coach voice". Keep the metrics; lead with what moved, read misses as circumstances under pressure, end with one kind next step. Never a report card; never moralize a slip.

## Instructions

### 0. Use the pre-built draft (v3.0)

A background `cadence-draft.sh weekly` job may have already written this period's review as a DRAFT: `Weekly Reviews/<YYYY-Www>.md`, frontmatter `status: draft`, auto content inside `<!-- BEGIN/END:auto-draft -->`. **If that draft exists**, make this session a thin **adjust → confirm**, not a regeneration: read it, present the `(proposed)` execution score + next-week targets (and any flags) for the user to adjust, then write their confirmed decisions into the human `## My Review` section (after the managed block) and flip `status: draft → final`. Re-gather only what's missing/stale; never rewrite the managed block's prose unless asked; never delete the file. If NO draft exists (job not enabled, or first run), fall back to the full gather below.
...
1. **[Work]** Use `gcal_list_events` (claude.ai Google Calendar integration) to get all events from Monday through Sunday. This covers you@workco.example calendars.
2. **[Personal]** Use `manage_calendar(operation: "agenda")` via the **google-personal** MCP server to get all events for the same Monday-through-Sunday range. This covers you@personal.example calendars.
...
- [partner] interaction low + Marriage & Home goal → suggest: "Plan this week's couple night. Q2 target: 1/week."
