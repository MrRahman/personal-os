...
**Tone (v3.0 Phase 2):** deliver the 0.0–1.0 goal scores, execution average, and retrospective in the **coach voice** from `.claude/skills/_conventions.md` → "Coach voice". Keep the scores honest; frame a low score as data for next quarter, not a judgment. Never a report card; never moralize.

## Instructions

### 0. Use the pre-built draft (v3.0)

A background `cadence-draft.sh quarterly` job may have already written this quarter's review as a DRAFT: `Quarterly Reviews/<YYYY-QX>.md`, frontmatter `status: draft`, auto content inside `<!-- BEGIN/END:auto-draft -->`, including a drafted next-quarter goal skeleton. **If that draft exists**, make this an **adjudicate → confirm**: read it, walk the `(proposed)` 0.0–1.0 goal scores + rationale one at a time for the user to adjudicate, then write confirmed scores + the reflective narrative into the human section (after the block) and flip `status: draft → final`. Shape the drafted next-quarter goals with the user. Never rewrite the managed block unless asked; never delete the file. If NO draft exists, fall back to the full session below.
