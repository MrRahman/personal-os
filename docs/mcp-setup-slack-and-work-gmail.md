...
**Date:** 2026-04-17
**Why:** The claude.ai web integrations for Work Gmail (you@workco.example) and Slack don't expose tools to Claude Code CLI. `/morning-plan` was silently missing the "Respond" (work email) and "Slack Later" sections. This runbook wires both into the project's stdio MCP config so they work in every Claude Code session.
...
{
  "accounts": [
    { "email": "you@personal.example", "category": "personal" },
    { "email": "you@workco.example", "category": "work" }
  ]
}
EOF
```
...
Log into WorkCo Slack in the Chromium window that opens. The script waits up to 5 minutes for login to complete, then extracts `xoxc` + `xoxd` and writes them into `.mcp.json` automatically. The profile at `~/.config/personal-os/slack-browser/` persists, so future refreshes run headless.
...
**On Enterprise Slack:** If WorkCo's Slack rejects the user-agent, add to the `slack` env block in `.mcp.json`:
...
1. Open `https://app.slack.com/client/...` (the WorkCo Slack workspace) in Chrome or Firefox.
...
Claude will invoke `mcp__google-personal__manage_accounts` with `{"operation": "authenticate"}`. A browser window opens. Sign in with `you@workco.example` and grant the requested scopes. The server writes credentials to `~/.local/share/google-workspace-mcp/credentials/you_at_workco_dot_com.json`.
...
### WorkCo Slack OAuth app (not needed for stealth mode, but for the record)
The stealth mode (xoxc + xoxd) in Step 3 doesn't require any admin install. If you ever want to switch to `xoxp` (User OAuth) you'd need WorkCo IT to either (a) whitelist a custom Slack app you create at api.slack.com/apps, or (b) allow installs from the Slack App Directory. Most orgs gate `search:read` scope behind IT approval because it can read anything the user can see. Ask pattern for IT:
...
### WorkCo Google Workspace OAuth (may block the authenticate flow)
When you run `manage_accounts authenticate` for you@workco.example, Google will show the consent screen for the OAuth client ID baked into `.mcp.json` (`<oauth-client-id>`). If WorkCo's Google Workspace admin has "Trust third-party apps" restricted (common at fintech/crypto companies), the flow will 403 with "This app is blocked by your administrator." Two fixes:

1. **Ask WorkCo IT to allowlist the OAuth client ID** — paste them the client ID above and the scopes requested (Gmail, Calendar read/write).
2. **Create a new OAuth client in a Google Cloud project you own**, mark it Internal only if WorkCo gives you GCP access, and replace `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` in `.mcp.json`. Requires WorkCo-provisioned GCP, which you likely don't have.
...
To undo everything: remove the `slack` block from `.mcp.json`, remove the you@workco.example entry from `accounts.json`, delete `~/.local/share/google-workspace-mcp/credentials/you_at_workco_dot_com.json`, and revert the CLAUDE.md MCP Services section. Git has the originals.
